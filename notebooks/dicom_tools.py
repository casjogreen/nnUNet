...
class DicomToolbox():
    ...
    @property    
    def __check_structure_coordinates(self):
        
        potential_bad_masks = []
        
        def check_coordinates(file, z, decimals):
                            
            with dcmread(file) as ds:  
                
                structures = self.read_structure(ds)
                mismatched_z_struct = False
                                
                for name in [c['name'] for c in structures]:

                    # grab z coordinates for the structure
                    contour_data = [np.array(i).reshape(-1,3) for i in [c['contour_data'] for c in structures if c['name'].lower() == name.lower()][0]]
                    z_struc = list(set([contour_data[i][0,2] for i in range(len(contour_data))]))
                    z_struc = np.round(np.sort(np.array(z_struc)), decimals)
                    
                    # detect if the mask has only one z-coordinate
                    if len(z_struc) == 1 and self.echo_level > 1: 
                        if name not in potential_bad_masks: self.__logger.warning(f'The mask "{name}" for pat-{self.patient_id} has only one z-coordinate.')
                        potential_bad_masks.append(name)
                        continue # skip to the next mask
                    
                    # check if the z-coordinates agree with the z coordinates of the CT for the good masks
                    if not all([z_struc[i] in np.round(z,decimals) for i in range(len(z_struc))]):
                        mismatched_z_struct = True
                        break # stop checking the rest of the masks if one is found to be bad
                                    
            if mismatched_z_struct and self.echo_level > 1: 
                self.__logger.warning(f'The z coordinates for at least one of the masks do not agree with those for the CT grid of pat-{self.patient_id}.')
            else:
                self.__logger.info(f'The z coordinates for all of the masks agree with those for the CT grid of pat-{self.patient_id}.')
                            
            return mismatched_z_struct
                                    
        # initial flag and coordinates
        for f in self.structure_files:
            
            mismatched_z_struct = check_coordinates(f, self.original_ct_coordinates.z, self.coordinate_precision)
        
            while mismatched_z_struct and self.coordinate_precision > self.min_coordinate_precision:            
                self.coordinate_precision -= 1
                check_coordinates(f, self.original_ct_coordinates.z, self.coordinate_precision)
                                    
        if mismatched_z_struct is False and self.echo_level > 1:
            self.__logger.info(f'Using a coordinate precision of {self.coordinate_precision} decimals for p-{self.patient_id}.')
        else:
            self.__logger.error(
                                "Unable to match the structure coordinates to the CT grid of pat-%s. "
                                "Please check the structure files using a TPS or something similar." % self.patient_id)